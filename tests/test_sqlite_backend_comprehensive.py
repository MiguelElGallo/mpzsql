...
class TestSQLiteBackendComprehensive:
    ...
    def test_init_read_only_mode(self):
        ...
        with pytest.raises(Exception):
            backend.execute_sql("INSERT INTO users (name, email) VALUES ('Test', 'test@test.com')")
    ...
    def test_execute_sql_multiple_statements(self):
        ...
        result = backend.connection.execute("SELECT COUNT(*) FROM multi_test").fetchone()
        assert result[0] == 2
    ...
    def test_execute_query_empty_result(self):
        ...
        assert isinstance(result, pa.Table)
        assert len(result) == 0
        assert result.schema.names == ['id', 'name', 'email', 'age', 'balance', 'is_active', 'created_at']
    ...
    def test_execute_query_with_null_values(self):
        ...
        result = backend.execute_query("SELECT name, email, age FROM users WHERE email IS NULL")
    ...
    def test_execute_update_update_statement(self):
        ...
        # Verify the updates
        result = backend.execute_query("SELECT age FROM users WHERE name = 'Alice Smith'")
    ...
    def test_execute_update_no_rows_affected(self):
        ...
        affected_rows = backend.execute_update("UPDATE users SET age = 30 WHERE name = 'Nonexistent'")
    ...
    def test_get_columns(self):
        ...
        assert isinstance(result, pa.Table)
        expected_columns = ['catalog_name', 'db_schema_name', 'table_name', 'column_name', 'data_type']
        assert result.schema.names == expected_columns
    ...
    def test_concurrent_access_thread_safety(self):
        ...
        for i, result in enumerate(results):
            assert result == expected_count, f"Thread {i} returned {result}, expected {expected_count}"
