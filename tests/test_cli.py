...
class TestValidationFunctions:
    ...
    def test_validate_tls_files_cert_not_found(self):
        ...
        with patch("pathlib.Path.exists", return_value=False):
            with pytest.raises(
                typer.BadParameter, match="TLS certificate file not found"
            ):
...
class TestPostgreSQLValidation:
    ...
    def test_validate_postgresql_connection_success(self, mock_console, mock_logger):
        ...
        with patch("psycopg2.connect", return_value=mock_conn):
            ...
            mock_console.print.assert_any_call(
                "[green]✅ PostgreSQL connection successful[/green]"
            )

    @patch("mpzsql.cli.get_main_logger")
    @patch("mpzsql.cli.console")
    def test_validate_postgresql_connection_azure_auth_success(
    ...
    @patch("mpzsql.cli.get_main_logger")
    @patch("mpzsql.cli.console")
    def test_validate_postgresql_connection_general_error(
        self, mock_console, mock_logger
    ):
        ...
        with patch("psycopg2.connect", side_effect=Exception("Connection failed")):
            ...
            assert result is False
            mock_console.print.assert_any_call(
                "[red]❌ PostgreSQL connection failed: Connection failed[/red]"
            )
...
class TestAzureStorageValidation:
    ...
    def test_validate_azure_storage_connection_success(
        self, mock_blob_client, mock_credential, mock_console, mock_logger
    ):
        ...
        assert result is True
        mock_console.print.assert_any_call(
            "[green]✅ Azure Storage connection successful[/green]"
        )

    @patch("mpzsql.cli.get_main_logger")
    @patch("mpzsql.cli.console")
    def test_validate_azure_storage_connection_import_error(
        self, mock_console, mock_logger
    ):
    ...
    @patch("azure.identity.DefaultAzureCredential")
    @patch("azure.storage.blob.BlobServiceClient")
    def test_validate_azure_storage_connection_general_error(
        self, mock_blob_client, mock_credential, mock_console, mock_logger
    ):
        ...
        assert result is False
        mock_console.print.assert_any_call(
            "[red]❌ Azure Storage connection failed: Connection failed[/red]"
        )
...
class TestAzureFilesystemSetup:
    ...
    async def test_setup_azure_filesystem_success(self, mock_filesystem, mock_console):
        ...
        mock_filesystem.assert_called_once_with(
            "abfs", account_name="testaccount", credential=credential
        )
    ...
    async def test_setup_azure_filesystem_failure(self, mock_filesystem, mock_console):
        ...
        mock_console.print.assert_any_call(
            "❌ Failed to setup Azure filesystem: Setup failed"
        )
    ...
    def test_setup_duckdb_connection_with_database(self, mock_connect, mock_console):
        ...
        mock_connect.assert_called_once_with(
            "test.db", config={"allow_unsigned_extensions": "true"}
        )
    ...
    def test_setup_duckdb_connection_failure(self, mock_connect, mock_console):
        ...
        mock_console.print.assert_any_call(
            "❌ Failed to setup DuckDB connection: Connection failed"
        )
...
class TestDuckDBInitialization:
    ...
    async def test_initialize_duckdb_with_azure_not_enabled(
        self, mock_basic_init, mock_console
    ):
    ...
    async def test_initialize_duckdb_with_azure_success(
        self, mock_credential_class, mock_setup_duckdb, mock_setup_azure, mock_console
    ):
    ...
    @patch("azure.identity.aio.DefaultAzureCredential")
    async def test_initialize_duckdb_with_azure_postgresql_enabled(
        self,
        ...
        assert any(
            "CREATE SECRET" in str(call) and "postgres" in str(call) for call in calls
        )
    ...
    @patch("mpzsql.cli.ensure_postgresql_database")
    @patch("azure.identity.aio.DefaultAzureCredential")
    async def test_initialize_duckdb_with_azure_postgresql_azure_auth(
        self,
        mock_credential_class,
        mock_ensure_db,
        mock_setup_duckdb,
    ...
    @patch("mpzsql.cli.initialize_duckdb_basic")
    @patch("azure.identity.aio.DefaultAzureCredential")
    async def test_initialize_duckdb_with_azure_failure_fallback(
        self, mock_credential_class, mock_basic_init, mock_console
    ):
...
class TestMainFunction:
    ...
    def test_main_invalid_server_config(
        self, mock_config_class, mock_logger, mock_logfire
    ):
    ...
    @patch("mpzsql.cli.validate_azure_storage_connection", return_value=True)
    def test_main_postgresql_connection_fails(
        self,
        mock_azure_validate,
        mock_pg_validate,
        mock_ensure_db,
    ...
    @patch("mpzsql.cli.validate_postgresql_connection", return_value=True)
    @patch("mpzsql.cli.validate_azure_storage_connection", return_value=True)
    def test_main_postgresql_database_creation_fails(
        self,
        mock_azure_validate,
        mock_pg_validate,
        mock_ensure_db,
    ...
    def test_main_azure_connection_fails(
        self,
        mock_azure_validate,
        mock_pg_validate,
        mock_ensure_db,
    ...
    @patch("mpzsql.cli.print_startup_banner")
    @patch("mpzsql.cli.MPZSQLServer")
    def test_main_with_azure_enabled(
        self,
        mock_server_class,
        mock_banner,
        mock_asyncio_run,
    ...
    @patch.dict(
        os.environ, {"WEBSITE_HOSTNAME": "azure_web_app.azurewebsites.net"}, clear=False
    )
    @patch("mpzsql.cli.LogfireManager")
    @patch("mpzsql.cli.get_main_logger")
    @patch("mpzsql.cli.ensure_postgresql_database", return_value=True)
    ...
    @patch("mpzsql.cli.print_startup_banner")
    @patch("mpzsql.cli.MPZSQLServer")
    def test_main_azure_web_app_hostname(
        self,
        mock_server_class,
        mock_banner,
        mock_init_db,
    ...
    def test_main_random_secret_generation(
        self,
        mock_token,
        mock_server_class,
        mock_banner,
...
class TestImportFallback:
    ...
    def test_import_fallback_coverage(self):
        ...
        from mpzsql.cli import __version__, app, console
...
class TestCLIEdgeCases:
    ...
    def test_init_sql_file_not_found_error(
        self, mock_read_text, mock_logger, mock_logfire
    ):
    ...
    def test_init_sql_file_permission_error(
        self, mock_read_text, mock_logger, mock_logfire
    ):
    ...
    def test_tls_key_file_not_found(self, mock_logger, mock_logfire):
        ...
        result = self.runner.invoke(
            cli.app, ["--tls-cert", "cert.pem", "--tls-key", "missing_key.pem"]
        )
