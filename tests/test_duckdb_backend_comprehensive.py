...
class TestDuckDBBackendComprehensive:
    ...
    def test_init_with_file_database(self):
        ...
        try:
            ...
            result = backend2.execute_query(
                "SELECT COUNT(*) as count FROM test_persistence"
            )
    ...
    def test_init_read_only_mode(self):
        ...
        try:
            ...
            result = backend.execute_query(
                "SELECT COUNT(*) as count FROM readonly_test"
            )
            assert result.to_pylist()[0]["count"] == 1
    ...
    def test_execute_update_delete_statement(self):
        ...
        backend.execute_sql(
            "INSERT INTO delete_test VALUES (1, true), (2, false), (3, true)"
        )

        affected_rows = backend.execute_update(
            "DELETE FROM delete_test WHERE active = false"
        )
    ...
    def test_execute_update_no_rows_affected(self):
        ...
        affected_rows = backend.execute_update(
            "UPDATE no_update_test SET id = 1 WHERE id = 999"
        )
    ...
    def test_get_statement_schema_simple_query(self):
        ...
        # Create test table
        backend.execute_sql(
            "CREATE TABLE schema_test (id INTEGER, name VARCHAR, age INTEGER)"
        )
    ...
    def test_get_columns(self):
        ...
        tables_result = backend.execute_query(
            "SELECT table_name FROM information_schema.tables WHERE table_name = 'column_test'"
        )
        if len(tables_result) == 0:
            pytest.skip(
                "Table not found in information_schema - may be DuckDB version specific"
            )
    ...
    def test_get_sql_info(self):
        ...
        assert result.schema.names == [
            "info_name",
            "value",
        ]  # DuckDB uses 'value', not 'info_value'
    ...
    def test_concurrent_access(self):
        ...
        for i in range(3):
            try:
                result = backend.execute_query(
                    "SELECT value FROM concurrent_test WHERE id = 1"
                )
