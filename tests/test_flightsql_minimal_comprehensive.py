...
class TestMinimalFlightSQLServerActions:
    ...
    def test_do_action_create_prepared_statement(
...
class TestMinimalFlightSQLServerFlightInfo:
    ...
    def test_get_flight_info_get_catalogs(self, server):
        ...
        with patch.object(
            FlightSQLProtobuf, "get_catalogs_schema", return_value=test_schema
        ):
            # Get flight info
            flight_info = server.get_flight_info(context, descriptor)
    ...
    def test_get_flight_info_unsupported_descriptor_type(self, server):
        ...
        with pytest.raises(
            NotImplementedError, match="Only CMD and PATH descriptors are supported"
        ):
    ...
    def test_get_flight_info_path_descriptor_support(self, server):
        ...
        # Mock backend methods used by PATH branch
        # Replace backend with a MagicMock to allow PATH-specific methods not in base spec
        path_backend = MagicMock()
        test_schema = pa.schema([pa.field("col1", pa.int64())])
        path_backend.get_table_schema.return_value = test_schema
        path_backend.get_table_row_count.return_value = 3
        server.backend = path_backend
    ...
    def test_get_flight_info_unparseable_command(self, server):
        ...
        with patch(
            "src.mpzsql.flightsql.protobuf.parse_any_command", return_value=None
        ):
