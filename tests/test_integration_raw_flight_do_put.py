...
@pytest.mark.skip(
    reason="Integration tests depend on unimplemented _handle_file_upload_do_put method"
)
class TestRawFlightDoPutIntegration:
    ...
    def test_complete_batch_upload_workflow(self) -> None:
        ...
        transformed_table_name = (
            f"my_ducklake.main.{table_name}"  # What FlightSQL server creates
        )
        ...
        # Execute the upload
        result = self.server._handle_file_upload_do_put(
        ...
        result = self.backend.execute_query(
            f"SELECT * FROM {transformed_table_name} ORDER BY order_id"
        )
    ...
    def test_complete_streaming_upload_workflow(self) -> None:
        ...
        result = self.backend.execute_query(
            f"SELECT COUNT(DISTINCT region) as regions FROM {transformed_table_name}"
        )
    ...
    def test_error_handling_integration(self) -> None:
        ...
        descriptor = pf.FlightDescriptor.for_path(b"nonexistent_table")
        ...
        descriptor = pf.FlightDescriptor.for_path(b"invalid_upload")
        reader = Mock()
    ...
    def test_table_replacement_integration(self) -> None:
        ...
        initial_data = pa.table({"id": [1, 2], "category": ["A", "B"]})
        ...
        new_data = pa.table({"id": [10, 20, 30], "category": ["X", "Y", "Z"]})
        ...
        result = self.backend.execute_query(
            f"SELECT id FROM {transformed_table_name} ORDER BY id"
        )
    ...
    def test_complex_data_types_integration(self) -> None:
        ...
        result = self.backend.execute_query(
            f"SELECT * FROM {transformed_table_name} WHERE bool_col = true"
        )
    ...
    def test_concurrent_operations_simulation(self) -> None:
        ...
        for table_name, transformed_table_name in tables_created:
            ...
            result = self.backend.execute_query(
                f"SELECT value FROM {transformed_table_name}"
            )
    ...
    def test_backward_compatibility(self) -> None:
        ...
        self.backend.execute_sql(
            "CREATE TABLE flightsql_test (id INTEGER, name VARCHAR)"
        )
