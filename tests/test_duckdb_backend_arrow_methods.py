...
class TestDuckDBBackendArrowMethods:
    ...
    def test_create_table_from_arrow_simple(self):
        ...
        schema_result = self.backend.execute_query(f"SELECT * FROM {table_name} LIMIT 0")
    ...
    def test_append_table_from_arrow_streaming_mode(self):
        ...
        data_result = self.backend.execute_query(f"SELECT id, value FROM {table_name} ORDER BY id")
    ...
    def test_complete_streaming_workflow(self):
        ...
        batch1 = pa.table({
            'batch_id': [1, 1, 1],
            'data': ['A', 'B', 'C'],
            'timestamp': [datetime.datetime(2024, 1, 1), datetime.datetime(2024, 1, 2), datetime.datetime(2024, 1, 3)]
        })
        ...
        batch2 = pa.table({
            'batch_id': [2, 2],
            'data': ['D', 'E'],
            'timestamp': [datetime.datetime(2024, 1, 4), datetime.datetime(2024, 1, 5)]
        })
        ...
        data_result = self.backend.execute_query(f"SELECT batch_id, data FROM {table_name} ORDER BY batch_id, data")
        data = data_result.to_pylist()
        assert len(data) == 5
    ...
    def test_large_table_performance(self):
        ...
        result = self.backend.execute_query(f"SELECT COUNT(DISTINCT category) as unique_cats FROM {table_name}")
        unique_categories = result.to_pylist()[0]['unique_cats']
        assert unique_categories == 10  # We create 10 categories in create_large_arrow_table
