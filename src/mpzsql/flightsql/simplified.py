...
class SimplifiedFlightSQL:
    ...
    def handle_action(self, action_type: str, action_body: bytes) -> pf.Result:
        ...
        logger.info(
            f"SimplifiedFlightSQL handling action: {action_type} ({len(action_body)} bytes)"
        )
            ...
            if len(data) > offset:
                try:
                    ...
                    if any(
                        ...
                        # Clean up the SQL
                        sql = "".join(
                            c for c in sql if ord(c) >= 32 or c in "\\t\\n\\r"
                        )
                        if len(sql) > 5:
                            return sql.strip()
                except Exception:
    ...
    def _handle_create_prepared_statement(self, action_body: bytes) -> pf.Result:
        ...
        try:
            ...
            if not sql:
                logger.error(
                    "Could not extract SQL from CreatePreparedStatement request"
                )
            ...
            logger.info(
                f"Returning {len(result_data)} bytes for prepared statement result"
            )
