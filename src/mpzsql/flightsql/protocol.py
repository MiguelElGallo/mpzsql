...
class FlightSQLSchemas:
    ...
    def get_catalogs_schema() -> pa.Schema:
        ...
        return pa.schema([pa.field("catalog_name", pa.utf8(), nullable=False)])
    ...
    def get_table_types_schema() -> pa.Schema:
        ...
        return pa.schema([pa.field("table_type", pa.utf8(), nullable=False)])
...
class FlightSQLMessageHandler:
    ...
    def encode_transaction_query(query: str, transaction_id: str = "") -> bytes:
        ...
        if transaction_id:
            return f"{transaction_id}:{query}".encode()
    ...
    def create_flight_info_for_query(
        query: str, schema: pa.Schema, location: pf.Location, transaction_id: str = ""
    ) -> pf.FlightInfo:
...
class FlightSQLServer:
    ...
    def prepare_query_for_get_tables(self, command) -> str:
        ...
        parts = [
            "SELECT 'main' as catalog_name, null as db_schema_name, name as table_name, type as table_type FROM sqlite_master where 1=1"
        ]
        ...
        if (
            hasattr(command, "db_schema_filter_pattern")
            and command.db_schema_filter_pattern
        ):
            parts.append(
                f" and db_schema_name LIKE '{command.db_schema_filter_pattern}'"
            )

        if (
            hasattr(command, "table_name_filter_pattern")
            and command.table_name_filter_pattern
        ):
    ...
    def prepare_query_for_get_imported_or_exported_keys(
        self, filter_clause: str
    ) -> str:
  ...
  pk_catalog_name, pk_db_schema_name, pk_table_name, pk_key_name, key_sequence"""
    ...
    def handle_flightsql_action(
        ...
        if action_type == FlightSQLCommands.ACTION_CLOSE_PREPARED_STATEMENT:
            return self._handle_close_prepared_statement(action_body)
        if action_type == FlightSQLCommands.ACTION_BEGIN_TRANSACTION:
            return self._handle_begin_transaction(action_body)
        if action_type == FlightSQLCommands.ACTION_END_TRANSACTION:
        ...
        if action_type == FlightSQLCommands.COMMAND_GET_SCHEMAS:
            return self._handle_get_schemas_action(action_body)
        if action_type == FlightSQLCommands.COMMAND_GET_TABLES:
            return self._handle_get_tables_action(action_body)
        if action_type == FlightSQLCommands.COMMAND_GET_TABLE_TYPES:
    ...
    def _handle_create_prepared_statement(self, action_body: bytes) -> pf.Result:
        ...
        try:
            ...
            self.prepared_statements[handle] = {"query": query, "parameters": []}
    ...
    def _handle_begin_transaction(self, action_body: bytes) -> pf.Result:
        ...
        try:
            ...
            # Store transaction
            self.open_transactions[transaction_id] = {"started": True}
    ...
    def _handle_get_catalogs_action(self) -> pf.Result:
        ...
        try:
            ...
            catalog_data = pa.table({"catalog_name": ["main"]})
    ...
    def _handle_get_schemas_action(self, action_body: bytes) -> pf.Result:
        ...
        try:
            ...
            schema_data = pa.table({"catalog_name": ["main"], "db_schema_name": [None]})
        ...
        if value and len(value) > 2:
            ...
            if pos < len(value) and value[pos] == 0x12:  # Field 2, string type
                ...
                if len(value) >= pos + 2 + length:
                    self.transaction_id = value[pos + 2 : pos + 2 + length].decode(
                        "utf-8"
                    )
