...
# Test both loggers
duckdb_logger.info(f"DuckDB logfire logger initialized - also writing legacy log to {log_path}")
...
class DuckDBBackend(DatabaseBackend):
    ...
    def execute_query(self, query: str, params: Optional[List] = None) -> pa.Table:
        ...
        try:
            ...
            duckdb_logger.info("Query executed successfully", rows=len(result), columns=len(result.schema))
    ...
    def execute_update(self, query: str, params: Optional[List] = None) -> int:
        ...
        try:
            ...
            if result.num_rows == 1 and result.num_columns == 1:
                ...
                if len(column) > 0:
                    ...
                    if scalar_value is not None:
                        ...
                        duckdb_logger.info("Update affected rows", affected_rows=affected_rows)
                        fh.flush()
            ...
            duckdb_logger.info("Update affected rows (fallback)", affected_rows=affected_rows)
            fh.flush()
            return affected_rows
            
        except Exception as e:
            duckdb_log.error(f"Error executing update query: {query}\n{e}")
            duckdb_logger.error("Update query execution failed", query=query, error=str(e))
            fh.flush()  # Force flush on error
            raise
    ...
    def get_statement_schema(self, query: str) -> pa.Schema:
        ...
        if (query_upper.startswith('USE ') or 
            ...
            query_upper.startswith('ATTACH ') or
            query_upper.startswith('DETACH ')):
    ...
    def get_catalogs(self) -> pa.Table:
        ...
        try:
            ...
            duckdb_log.info(f"get_catalogs() - Query returned {len(catalogs)} catalogs: {catalogs}")
            ...
            if not catalogs:
                ...
                result = self.connection.execute(query).fetchall()
                catalogs = [row[0] for row in result]
                duckdb_log.info(f"get_catalogs() - Fallback query returned {len(catalogs)} catalogs: {catalogs}")
            ...
            # Convert to compatible types for JDBC driver
            table = self._convert_large_utf8_to_utf8(table)
            duckdb_log.info(f"get_catalogs() - Returning default table with {len(table)} rows")
            fh.flush()
            return table
            ...
            try:
                ...
                for cat in catalogs:
                    ...
                    try:
                        ...
                        result_schemas.extend([(cat, "main"), (cat, "information_schema")])
                ...
                logger.error(f"Schema fallback failed: {e2}")
                return [("main", "main"), ("main", "information_schema")]  # Default schemas
    ...
    def get_tables(
        ...
        db_schema_filter_pattern = db_schema_filter_pattern.replace("*", "%") if db_schema_filter_pattern else None
        table_name_filter_pattern = table_name_filter_pattern.replace("*", "%") if table_name_filter_pattern else None
        ...
        if catalog is not None:
            ...
            duckdb_log.info("get_tables() - No catalog specified, using CURRENT_DATABASE() filter")
        ...
        try:
            ...
            duckdb_log.info(f"get_tables() - Query returned {result_table.num_rows} tables")
            ...
            if result_table.num_rows > 0:
                ...
                for i, (cat, schema, table, ttype) in enumerate(zip(catalogs, schemas, tables, table_types)):
            ...
            if include_schema:
                ...
                for table_row in result_table.to_pylist():
                    ...
                    try:
                        ...
                        duckdb_log.info(f"Executing schema query for {table_name}: {schema_query}")
                        ...
                        duckdb_log.info(f"Got Arrow schema for {table_name}: {arrow_schema}")
                        ...
                        duckdb_log.info(f"Successfully serialized schema for table {table_name} ({len(schema_bytes)} bytes)")
                    except Exception as e:
                        duckdb_log.error(f"Failed to get schema for table {table_name}: {e}")
                ...
                final_schema = pa.schema(base_fields + [pa.field("table_schema", pa.binary())])
                result_table = result_table.cast(final_schema)
                duckdb_log.info(f"get_tables() - Successfully added 'table_schema' column. Final table has {result_table.num_rows} rows and {result_table.num_columns} columns")
    ...
    def get_columns(
        ...
        try:
            ...
            if db_schema_filter_pattern:
                conditions.append(f"table_schema LIKE '{db_schema_filter_pattern.replace('*', '%')}'")
            if table_name_filter_pattern:
                conditions.append(f"table_name LIKE '{table_name_filter_pattern.replace('*', '%')}'")
            if column_name_filter_pattern:
                conditions.append(f"column_name LIKE '{column_name_filter_pattern.replace('*', '%')}'")
            ...
            # Ref: java.sql.Types
            sql_type_map = {
                'BIGINT': -5, 'BOOLEAN': 16, 'BLOB': 2004, 'DATE': 91,
                'DOUBLE': 8, 'INTEGER': 4, 'FLOAT': 6, 'REAL': 7,
                'SMALLINT': 5, 'TIME': 92, 'TIMESTAMP': 93, 'TINYINT': -6,
                'VARCHAR': 12, 'UUID': -11, 'INTERVAL': -10, 'DECIMAL': 3,
                'LIST': 2003, 'STRUCT': 2002, 'MAP': 2000,
            }
            ...
            data_type_values = [map_type_name_to_sql_type(s.as_py()) for s in type_name_col]
            ...
            nullable_values = [1 if s and s.as_py() == 'YES' else 0 for s in is_nullable_col_str]
    ...
    def get_tables_filtered(self, filters: dict) -> pa.Table:
        ...
        try:
            ...
            if result.num_rows == 0 and filters.get('table_name_pattern', '').startswith('pg_'):
                logger.info(f"No PostgreSQL system table found: {filters['table_name_pattern']}")
    ...
    def get_sql_info(self, info_codes: List[int]) -> pa.Table:
        ...
        value_array = pa.array([f"value_{code}" for code in info_codes], type=pa.string())
        
        schema = pa.schema([
            pa.field("info_name", pa.int32()),
            pa.field("value", pa.string())
        ])
        
        return pa.Table.from_arrays([info_name_array, value_array], schema=schema)

    def get_db_schemas(self, catalog: Optional[str] = None, db_schema_filter_pattern: Optional[str] = None) -> pa.Table:
        ...
        try:
            ...
            if catalog is not None:
                ...
                query += " AND catalog_name = CURRENT_DATABASE()"
                duckdb_log.info("get_db_schemas() - No catalog specified, using CURRENT_DATABASE() filter")
