...
class MPZSQLClient:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.certificate:
                ...
                console.print(f"[blue]🔐 Connecting to FlightSQL server at {uri} with TLS...[/blue]")
            else:
                # Use plain TCP
                uri = f"grpc://{self.host}:{self.port}"
                console.print(f"[blue]🔗 Connecting to FlightSQL server at {uri}...[/blue]")
            ...
            if self.certificate:
                ...
                if cert_path.exists():
                    ...
                    console.print(f"[red]❌ Certificate file not found: {self.certificate}[/red]")
                    return False
            ...
            if self.username and self.password:
                console.print(f"[blue]🔑 Authenticating as user: {self.username}[/blue]")
                # Use the working ADBC authentication method
                auth_header = base64.b64encode(f"{self.username}:{self.password}".encode()).decode()
                db_kwargs[DatabaseOptions.AUTHORIZATION_HEADER.value] = f"Basic {auth_header}"
            ...
            self.connection = flightsql_dbapi.connect(
                uri,
                db_kwargs=db_kwargs if db_kwargs else None
            )
            
            console.print("[green]✅ Connected to FlightSQL server with TLS + Authentication[/green]")
            return True
    ...
    def execute_query(self, sql: str) -> Optional[pa.Table]:
        ...
        try:
            ...
            console.print(f"[green]✅ Query executed successfully. Rows: {table.num_rows}[/green]")
            return table
    ...
    def execute_update(self, sql: str) -> bool:
        ...
        try:
            ...
            console.print(f"[green]✅ Statement executed successfully[/green]")
            return True
...
def connect(
    ...
    username: Optional[str] = typer.Option(None, "--user", "-u", help="Username for authentication"),
    password: Optional[str] = typer.Option(None, "--password", "-P", help="Password for authentication"),
    certificate: Optional[str] = typer.Option(None, "--cert", "-c", help="Path to TLS certificate file"),
    interactive: bool = typer.Option(True, "--interactive/--no-interactive", "-i", help="Start interactive mode"),
):
...
def _interactive_mode(client: MPZSQLClient):
    ...
    while True:
        try:
            ...
            if command.lower() in ['quit', 'exit', 'q']:
            ...
            elif command.lower().startswith('select') or command.lower().startswith('show'):
                ...
                console.print("[yellow]Empty command. Type 'help' for available commands.[/yellow]")
...
@app.command()
def execute(
    sql: str = typer.Argument(..., help="SQL statement to execute (CREATE, INSERT, UPDATE, DELETE)"),
    host: str = typer.Option("127.0.0.1", "--host", "-h", help="Server host"),
    port: int = typer.Option(8080, "--port", "-p", help="Server port"),
    username: Optional[str] = typer.Option(None, "--user", "-u", help="Username for authentication"),
    password: Optional[str] = typer.Option(None, "--password", "-P", help="Password for authentication"),
